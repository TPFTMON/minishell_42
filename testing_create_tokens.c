...
// create_token + add_token_back + free_token_list
int main(void)
{
	t_token	*tokens;
	t_token	*token1;
	...
	tokens = NULL;
	token1 = create_token(TOKEN_PIPE, "|");
	if (!token1)
		printf("The what now????");
	add_token_back(&tokens, token1);
	token2 = create_token(TOKEN_WORD, "miaou");
	if (!token2)
		printf("The what now????");
	add_token_back(&tokens, token2);
	token3 = create_token(TOKEN_WORD, "wc");
	if (!token3)
		printf("The what now????");
