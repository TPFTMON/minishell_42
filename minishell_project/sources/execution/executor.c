...
/*   Updated: 2025/07/14 19:19:49 by sguan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_builtin_with_redir(t_shell_data *shell, char **args,
		t_redirection *redir)
{
	int	original_stdin;
	int	original_stdout;
...
int	execute_single_command(t_shell_data *shell, t_command *cmd)
{
    ...
    {
        if (cmd->redirections)
            return (execute_builtin_with_redir(shell, cmd->argv, cmd->redirections));
        else
